...
ELMOSLM = Token:match("(%d+)")
redis:set(ELMOSLM..":VERSION",1)
redis:set(ELMOSLM..":SUDO_ID:",SUDO_USER)
redis:set(ELMOSLM..":DataCenter:",'German')
redis:set(ELMOSLM..":UserNameBot:",BOT_User)
redis:set(ELMOSLM..":NameBot:",BOT_NAME)
redis:hset(ELMOSLM..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3ELMOSLM_INSTALL","Yes")
info = {}
...
ELMOSLM = Token:match("(%d+)")
our_id = tonumber(ELMOSLM)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(ELMOSLM..":UserNameBot:")
SUDO_ID = tonumber(redis:get(ELMOSLM..":SUDO_ID:"))
SUDO_USER = redis:hgetall(ELMOSLM..'username:'..SUDO_ID).username
version = redis:get(ELMOSLM..":VERSION")
DataCenter = redis:get(ELMOSLM..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..ELMOSLM..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(ELMOSLM..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(ELMOSLM..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(ELMOSLM..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

elseif msg.sender_user_id_ == 1236115319 then 
msg.TheRank =  ' مبرمج السورس '
msg.Rank = 1

elseif msg.sender_user_id_ == 835551362 then 
...
msg.Rank = 1
elseif redis:sismember(ELMOSLM..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(ELMOSLM..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(ELMOSLM..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(ELMOSLM..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(ELMOSLM..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(ELMOSLM..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(ELMOSLM..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(ELMOSLM..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(ELMOSLM..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(ELMOSLM..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(ELMOSLM..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(ELMOSLM..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس المسلم ⇐ @suorselmoslam
...
if msg.GroupActive and redis:get(ELMOSLM..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(ELMOSLM..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(ELMOSLM..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(ELMOSLM..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.ELMOSLM then 
if msg.text and ScriptFile.iELMOSLM then
for k, ELMOSLM in pairs(ScriptFile.ELMOSLM) do
local SearchText = msg.text:match(ELMOSLM)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iELMOSLM(msg,{msg.text:match(ELMOSLM)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ELMOSLM.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dELMOSLM then
if ScriptFile.dELMOSLM(msg) == false then
return false
end
...
if Plug.ELMOSLM then 
if msg.text and Plug.iELMOSLM then
for k, ELMOSLM in pairs(Plug.ELMOSLM) do
local SearchText = msg.text:match(ELMOSLM)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(ELMOSLM..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(ELMOSLM..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iELMOSLM(msg,{msg.text:match(ELMOSLM)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ELMOSLM.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dELMOSLM then
Plug.dELMOSLM(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source ELMOSLM")
end 

end
else
print("The File Script.lua Not Runing in The Source ELMOSLM")
...
function tdcli_update_callback(data)
	...
	redis:srem(ELMOSLM..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELMOSLM/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELMOSLM/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELMOSLM/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELMOSLM/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELMOSLM/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELMOSLM/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(ELMOSLM..":SUDO_ID:")
redis:hset(ELMOSLM..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(ELMOSLM..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(ELMOSLM..'mute_tgservice'..msg.chat_id_) then
	Del_msg(msg.chat_id_,msg.id_)
	end
	elseif msg.content_.ID == "MessageChatAddMembers" then
	if redis:get(ELMOSLM..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(ELMOSLM..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(ELMOSLM..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ELMOSLM..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ELMOSLM..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(ELMOSLM..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(ELMOSLM..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(ELMOSLM..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(ELMOSLM..'group:add'..msg.chat_id_) then
	redis:set(ELMOSLM..'group:name'..msg.chat_id_,msg.content_.title_)
	end
	end 
	
	elseif data.ID == "UpdateNewChat" then  
	if redis:get(ELMOSLM..'group:add'..data.chat_.id_) then
	redis:set(ELMOSLM..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(ELMOSLM..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(ELMOSLM..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(ELMOSLM..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	ELMOSLM..'group:name'..k,IDS.Title,
	ELMOSLM..'num_msg_ELMOSLM'..k,5,
	ELMOSLM..'group:add'..k,true,
	ELMOSLM..'lock_link'..k,true,
	ELMOSLM..'lock_spam'..k,true,
	ELMOSLM..'lock_webpage'..k,true,
	ELMOSLM..'lock_markdown'..k,true,
	ELMOSLM..'lock_flood'..k,true,
	ELMOSLM..'lock_bots'..k,true,
	ELMOSLM..'mute_forward'..k,true,
	ELMOSLM..'mute_contact'..k,true,
	ELMOSLM..'mute_document'..k,true,
	ELMOSLM..'mute_inline'..k,true,
	ELMOSLM..'lock_username'..k,true,
	ELMOSLM..'replay'..k,true
	)
	redis:sadd(ELMOSLM..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(ELMOSLM..'username:'..ID,'username',user)
	redis:sadd(ELMOSLM..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(ELMOSLM..'username:'..ID,'username',user)
	redis:sadd(ELMOSLM..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(ELMOSLM..'username:'..ID,'username',user)
	redis:sadd(ELMOSLM..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(ELMOSLM..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(ELMOSLM..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(ELMOSLM..'username:'..data.user_.id_).username
	...
	redis:hset(ELMOSLM..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
	...
	local groups = redis:smembers(ELMOSLM..'group:ids')
	...
	local pv = redis:smembers(ELMOSLM..'users')
	local NumPvDel = 0
	for i = 1, #pv do
